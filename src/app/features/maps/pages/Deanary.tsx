...
export default function Deanary() {
  ...
  useEffect(() => {
    async function loadData() {
      const cellRes = await fetch("/data/maps/cells/deanery_lvl_2_cells.json");
      const layoutRes = await fetch("/data/maps/layouts/deanery_lvl_2_layout.json");
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-hidden">
      <div className='flex justify-between items-center mb-4'>
        <h2 className="text-xl font-bold mb-4">Deanary Level 2</h2>
        <button
          onClick={handleSave}
