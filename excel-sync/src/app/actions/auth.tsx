...
import { createSession, deleteSession, decrypt } from '@/lib/session' // Import session management functions
...
export async function signup(state: FormState, formData: FormData) {
  ...
  const existingUser = await prisma.user.findFirst({ // Query the database for existing users
    where: {
      OR: [
        { name: username }, // Check if username exists
  ...
  if (existingUser) { // If user already exists
    return {
      errors: {
        username: ['Username already exists!'], // Return username error
        email: ['Email already exists!'], // Return email error
      }
    }
  }
  ...
  return {
    message: 'User created successfully!' // Return success message
  }
}
...
export async function signin(state: FormState, formData: FormData) {
  ...
  if (!user) { // If user does not exist
    return {
      errors: {
        username: ['User is not registered!'], // Return username error
      }
    }
  }
  ...
  if (!isMatch) { // If passwords do not match
    return {
      errors: {
        password: ['Password is incorrect!'], // Return password error
      }
    }
  }
